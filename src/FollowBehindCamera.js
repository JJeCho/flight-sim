// FollowBehindCamera.jsx
import { useFrame } from '@react-three/fiber';
import { useEffect, useRef } from 'react';
import { PerspectiveCamera } from '@react-three/drei';
import * as THREE from 'three';

function FollowBehindCamera({ planeApi, makeDefault }) {
  ...
  useFrame(() => {
    ...
    const followOffset = new THREE.Vector3(0, 5, 2);

    const planeQuat = new THREE.Quaternion(qx, qy, qz, qw);
    const offsetPosition = followOffset.clone().applyQuaternion(planeQuat);
    ...
    const lookAtTarget = new THREE.Vector3(px, py + 1, pz);
...
export default FollowBehindCamera;
